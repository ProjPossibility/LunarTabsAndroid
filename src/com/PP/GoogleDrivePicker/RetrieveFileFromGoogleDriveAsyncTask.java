    ...
    protected Boolean doInBackground(DriveId... params) {
    	try {
	        ...
	        ContentsResult contentsResult =
	                file.openContents(mGoogleApiClient, DriveFile.MODE_READ_ONLY, null).await();
	        if (!contentsResult.getStatus().isSuccess()) {
           	 	TextToSpeechAPI.speak(ResourceModel.getInstance().ERROR_FILE_NOT_LOADED);	        	
	        	return false;
	        }        
        	final TGSong song = TuxGuitarUtil.loadSong(contentsResult.getContents().getInputStream());
